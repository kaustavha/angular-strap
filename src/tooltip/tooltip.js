  ...
  .provider('$tooltip', function() {
    ...
    this.$get = function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $timeout, dimensions) {
      ...
      function TooltipFactory(element, config) {
        ...
        var templatePromise = $q.when($templateCache.get(options.template) || $http.get(options.template/*, {cache: true}*/));
        var scope = options.scope.$new() || $rootScope.$new();
        if(options.delay && isString(options.delay)) {
        ...
        templatePromise.then(function(template) {
          if(isObject(template)) template = trim.apply(template.data);
